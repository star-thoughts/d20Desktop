...
namespace d20Web.Helpers
{
    ...
    public static class Dice
    {
        ...
        {
            return dieString.IsInteger() || dieString.Split(new char[] { 'd', 'D' }, StringSplitOptions.RemoveEmptyEntries).All(p => p.IsInteger());
        }
        ...
        private static int RollOrAdd(string part, RollingStrategy strategy)
        {
            ...
            {
                string[] numbers = part.Split(new char[] { 'd', 'D' }, StringSplitOptions.RemoveEmptyEntries);
                if (numbers.Length != 2)
                    throw new ArgumentException("Unable to parse die string", part);
                if (!int.TryParse(numbers[0], NumberStyles.Integer, CultureInfo.InvariantCulture, out int times))
                    throw new ArgumentException("Unable to parse die string", part);
                if (!int.TryParse(numbers[1], NumberStyles.Integer, CultureInfo.InvariantCulture, out int die))
                    throw new ArgumentException("Unable to parse die string", part);
        ...
        private static bool IsInteger(this string s)
        {
            return int.TryParse(s, NumberStyles.Integer, CultureInfo.CurrentCulture, out int _);
        }
    }
}
